...
adm3_shp <- read_sf(here('data', 'input', 'shp', 'ago_admbnda_adm3_gadm_ine_ocha_20180904.shp')) 
...
ang_map <- ggplot() +
  geom_sf(fill = "transparent", color = "blue", size = 1,
          data = ang_shp) +
    geom_sf(fill = "transparent", color = "red", size = 1.5, 
          data = adm1_shp) +
...
ang_raster_map <- ggplot(ang_shp) +
  geom_sf() +
  geom_raster(data = angola_oncho_point_df, aes(lon, lat, fill = suitability), alpha = .7) +
...
unknown_iu <- ang_espen_data %>% filter(grepl('Unknown', Endemicity)) %>% pull(IU_ID)
...
lf_endemicity_map <- ggplot() +
  geom_sf(data=ang_lf_endemicity_data,aes(fill = lf_status), color = "gray50", size = 0.5) +
  geom_sf(fill = "transparent", color = "black", size = 1.5,
          data = adm1_shp) +
...
oncho_unknown_ius <- oncho_wd %>% filter(grepl("Unknown",oncho_status)) %>% pull(IU_ID)
...
oncho_loaloa_lf_combined_map2 <- ggplot() +
  geom_sf(data=ang_endemicity_data %>% filter(!is.na(final_endemicity)),
          aes(fill = final_endemicity), color = "gray50", size = 0.5) +
    geom_sf(fill = "transparent", color = "black", size = 1.5, 
          data = adm1_shp) +
        scale_fill_discrete(na.translate = F) +
  labs(fill='Loaloa-LF') + 
...
cachiungo_oncho_map <- oncho_angola %>% 
  ...
  ggplot() +
  geom_sf(aes(fill=oncho_mean_aw), lwd=1) +
  gghighlight(IUs_NAME=='Cachiungo') +
  geom_sf_label_repel(aes(label = IUs_NAME),
                      force = 100, nudge_x = -2, seed = 10) +
  labs(fill='Environmental suitability') +
  theme(legend.position='top') +
