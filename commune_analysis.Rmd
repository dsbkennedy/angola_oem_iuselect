...
adm2_unknown <- iu_commune %>% 
  #pull(ADM2_EN)
  as_tibble() %>% 
  select(adm1_en,adm2_en,Endemicity, loaloa_status) 
...
unknown_commune_shp <- commune_shp %>% 
  ...
  right_join(adm2_unknown,by=c('adm1_en','adm2_en'))

 # right_join(adm2_unknown,by=c('ADM1_EN'='ADMIN1_com', 'ADM2_EN'='ADMIN2_com'))

unknown_commune_df <- unknown_commune_shp %>% as_tibble() %>% select(adm1_en, adm2_en, adm3_en) %>% mutate(spatial_join=1)
...
iu_commune_map_upd <- commune_shp_unknown %>%
  ggplot() +
